...
@section('breadcrumb', 'Noticias → Editar')
@section('content')
@php $initType = old('type', $post->type); @endphp
<div class="max-w-4xl mx-auto" x-data="postFormEditor('{{ $initType }}')">
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-extrabold text-white">Editar Artículo</h1>
            <a href="{{ route('admin.posts.index') }}" class="text-gray-500 text-xs hover:text-[#00d4ff]">← Volver a artículos</a>
        </div>
        <a href="{{ route('posts.show', $post->slug) }}" target="_blank"
           class="text-[#00d4ff] text-sm hover:underline">Ver en el sitio →</a>
    </div>
    ...
    <form method="POST" action="{{ route('admin.posts.update', $post) }}" enctype="multipart/form-data"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div>
                ...
                <select name="type" required x-model="postType" @change="onTypeChange"
                        class="w-full bg-[#0a0e1a] border border-[#1e2a3a] text-white rounded-lg px-3 py-2 text-sm focus:border-[#00d4ff] focus:outline-none">
                    <option value="news"  {{ $initType === 'news'  ? 'selected' : '' }}>Noticia</option>
                    <option value="guide" {{ $initType === 'guide' ? 'selected' : '' }}>Guía</option>
                </select>
            </div>
            <div>
                <label class="block text-gray-400 text-sm mb-1">Imagen de portada</label>
                @if($post->image)
                    <img src="{{ asset('storage/' . $post->image) }}" alt="Portada actual"
                         class="w-24 h-16 object-cover rounded mb-2">
                    <p class="text-gray-500 text-xs mb-1">Sube una nueva para reemplazar</p>
                @endif
            ...
            <div class="md:col-span-2 flex flex-wrap gap-6">
                ...
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" name="is_slider" value="1" {{ old('is_slider', $post->is_slider) ? 'checked' : '' }}
                           class="rounded border-[#1e2a3a] bg-[#0a0e1a] text-[#00d4ff]">
                    <span class="text-gray-300 text-sm">◉ Slider</span>
                </label>
            </div>
        </div>
        ...
        <div x-show="tab === 'es'" class="space-y-5">
            ...
            <div>
                ...
                <textarea name="excerpt" rows="2"
                          class="w-full bg-[#0a0e1a] border border-[#1e2a3a] text-white rounded-lg px-3 py-2 text-sm focus:border-[#00d4ff] focus:outline-none resize-none">{{ old('excerpt', $post->excerpt) }}</textarea>
            </div>
        ...
        {{-- ── EN TAB ──────────────────────────────────────── --}}
        @php $enTrans = $post->translations->firstWhere('locale', 'en'); @endphp
        <div x-show="tab === 'en'" class="space-y-5">
            ...
            <div>
                ...
                <input type="text" name="trans_en[title]"
                       value="{{ old('trans_en.title', $enTrans?->title ?? '') }}"
                       x-model="enTitle"
                       class="w-full bg-[#0a0e1a] border border-[#1e2a3a] text-white rounded-lg px-3 py-2 text-sm focus:border-[#00d4ff] focus:outline-none"
...
// Build EN section data from existing DB translations
@php
    $enTrans = $post->translations->firstWhere('locale', 'en');
    $enSecMap = [];
    foreach ($post->guideSections as $sec) {
        $t = $sec->translations->firstWhere('locale', 'en');
        $enSecMap[$sec->id] = ['title' => $t?->title ?? '', 'content' => $t?->content ?? ''];
    }
@endphp
const _existingSections = @json($post->guideSections->map(fn($s) => ['id' => $s->id, 'title' => $s->title, 'content' => $s->content])->values());
const _existingEnSections = @json(array_values($enSecMap));

function postFormEditor(initialType) {
    return {
        postType: initialType,
        tab: 'es',
        enTitle: '{{ old('trans_en.title', $post->translations->firstWhere('locale', 'en')?->title ?? '') }}',
        sections: [],
        enSections: [],
        _uidCtr: 0,
        ...
        init() {
            const esSrc = @if(old('sections')) @json(old('sections', [])) @else _existingSections @endif;
            const enSrc = @if(old('trans_en.sections')) @json(old('trans_en.sections', [])) @else _existingEnSections @endif;

            esSrc.forEach((s, idx) => {
                const uid = 'gse-ed-' + (this._uidCtr++);
                this.sections.push({ uid, title: s.title ?? '', content: s.content ?? '' });
                this.enSections.push({ title: enSrc[idx]?.title ?? '', content: enSrc[idx]?.content ?? '' });
                this.$nextTick(() => {
                    initSecEditor(uid, s.content ?? '');
                    initSecEditor('en-' + uid, enSrc[idx]?.content ?? '');
                });
            });
        },
