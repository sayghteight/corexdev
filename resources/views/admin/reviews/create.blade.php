...
@section('title', 'Nueva Reseña — Admin')

@section('breadcrumb', 'Rese�as � Nueva')
@section('content')
<div class="max-w-4xl mx-auto">
    <div class="mb-6">
        <h1 class="text-2xl font-extrabold text-white">Nueva Reseña</h1>
    ...
    <form method="POST" action="{{ route('admin.reviews.store') }}" enctype="multipart/form-data"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
                <label class="block text-gray-400 text-sm mb-1">Nombre del Juego *</label>
                <input type="text" name="game" value="{{ old('game') }}" required
            ...
            <div>
                <label class="block text-gray-400 text-sm mb-1">Puntuación (0-10) *</label>
                <input type="number" name="score" value="{{ old('score') }}" min="0" max="10" step="0.1" required
                       class="w-full bg-[#0a0e1a] border border-[#1e2a3a] text-white rounded-lg px-3 py-2 text-sm focus:border-[#ffd700] focus:outline-none">
            </div>
            <div>
            ...
            <div class="md:col-span-2">
                <label class="flex items-center gap-2 cursor-pointer">
                    ...
                    <span class="text-gray-300 text-sm"><i class="fa-solid fa-star text-[#ffd700] mr-1"></i> Destacar en home</span>
                </label>
            </div>
        </div>

        {{-- Language Tab Switcher + Content --}}
        <div x-data="{ tab: 'es', enTitle: '{{ old('trans_en.title', '') }}' }" class="space-y-5">
        ...
        <div x-show="tab === 'es'" class="space-y-5">
            ...
            <div>
                ...
                <textarea name="excerpt" rows="2"
                          class="w-full bg-[#0a0e1a] border border-[#1e2a3a] text-white rounded-lg px-3 py-2 text-sm focus:border-[#ffd700] focus:outline-none resize-none">{{ old('excerpt') }}</textarea>
            </div>
            <div>
        ...
        {{-- Pros / Cons (ES) --}}
        <div class="border border-[#1e2a3a] rounded-xl p-5 space-y-4" x-data="highlightsEditor()">
